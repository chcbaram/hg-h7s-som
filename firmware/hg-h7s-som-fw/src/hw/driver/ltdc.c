...
__attribute__((section(".lcd_buf"))) __attribute__((aligned(64)))
...
bool ltdcInit(void)
{
  ...
  for (int ch=0; ch<FRAME_BUF_CNT; ch++)
  {
    ...
    {
      frame_buffer[ch][i] = 0xF000;
    }
  }
  logPrintf("[  ] Lcd Clear %d ms\n", millis()-pre_time);
  ...
  uint32_t ltdc_clock;
  ltdc_clock = HAL_RCCEx_GetPeriphCLKFreq(RCC_PERIPHCLK_LTDC);
  logPrintf("     freq : %d.%03d Mhz\n", 
    ltdc_clock/1000000,
    (ltdc_clock%1000000)/1000
    );
